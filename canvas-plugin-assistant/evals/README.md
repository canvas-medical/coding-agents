...
This directory contains eval cases for testing the CPA review commands (`/cpa:security-review` and `/cpa:database-performance-review`).
...
Use the `/cpa:run-evals` command to execute all eval cases and generate a results report.

**IMPORTANT:** The eval runner does NOT read expected.json or case_index.md before running reviews - this ensures blind evaluation. Both `/cpa:security-review` and `/cpa:database-performance-review` are run on each case, and the comparison script determines which findings are relevant.
