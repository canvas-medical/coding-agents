...
- [assets/canvas-plugin-ui.js](assets/canvas-plugin-ui.js) — All 27 web components bundled into a single script
...
- [references/writing-style.md](references/writing-style.md) — Copy rules for every user facing string, banned punctuation and vocabulary, clinical vocabulary carve out for vital, critical, significant, active, acute, and related terms
...
- All 27 components have documented APIs in [references/web-components.md](references/web-components.md). Check the API before using any component. Do not write raw HTML for elements that have a web component. This includes `<input>`, which is always `canvas-input`, `<select>`, which is always `canvas-dropdown` or `canvas-combobox`, and `<details>` with `<summary>`, which is always `canvas-accordion` with `canvas-accordion-item`, `canvas-accordion-title`, and `canvas-accordion-content`.
- Action menus are not dropdowns. A button that opens a list of verbs (Edit, Duplicate, Delete, Add X, Add Y) is a `canvas-menu-button`, not a `canvas-dropdown`. The dropdown element is form associated with a `value` and a `change` event. The menu button carries no form state and dispatches a `select` event per action. Wrong choice breaks ARIA, confuses screen readers, and surfaces form fields that do not exist. See Menu Button in [references/component-usage.md](references/component-usage.md).
- Anchored content surfaces are not modals. A button that opens a panel tied to that specific trigger (filter form, column picker, legend, bulk action panel) is a `canvas-popover`, not a `canvas-modal`. Popovers anchor to the trigger, follow it on scroll, and use `role="dialog"` with `aria-modal="false"`. They are non blocking by contract, outside click and Escape always dismiss. Destructive confirmations and blocking flows that the user must respond to before continuing are not popovers, they are `canvas-modal`. See Popover in [references/component-usage.md](references/component-usage.md).
...
- User facing strings follow [references/writing-style.md](references/writing-style.md). No em dashes, sentence case headings, no AI puffery (vibrant, boasts, serves as, seamlessly), no didactic disclaimers (It is important to note), no collaborative assistant tone (Let me help you). The clinical vocabulary carve out in writing-style.md preserves vital signs, critical values, clinically significant findings, active medications, acute and chronic conditions, pivotal trials, presenting complaints, and related domain terms in their clinical sense.
