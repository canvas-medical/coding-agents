...
- The accordion header has two regions split by interactivity. The caret arrow is always first and never moves. `canvas-accordion-title` holds the label and is the click target, it renders inside the trigger button so it takes only non-interactive content, text and an optional `canvas-badge`, sized to its content and aligned to the start so the badge sits right beside the label. Interactive controls go in `canvas-accordion-actions`, a sibling of the button that pins to the far right of the header. A toggle, checkbox, radio, button, or any other control belongs in the actions slot, never in the title.
- Never place an interactive control inside `canvas-accordion-title`. A control inside the trigger button is a nested-interactive accessibility violation and is unreliable for screen readers. Put it in `canvas-accordion-actions` instead, where it renders outside the button, stays reachable, and its clicks never toggle the panel, with no `event.stopPropagation` needed. See [anti-patterns.md](anti-patterns.md) Interactive Control in Accordion Title.
- The default title height is ~34.58px. Do not add `min-height` or extra padding to accordion titles. If a larger trigger height is needed, the user must explicitly request it. When toggles or checkboxes sit in the actions slot, they naturally expand the row through their own height.
...
- Use `canvas-input` for single-line values like names, emails, phone numbers, dates, and short identifiers. For phone numbers, set `format="phone"` and `type="tel"` so the value masks to `(000) 000-0000` while `value` stays raw digits. See [web-components.md](web-components.md) canvas-input Phone formatting.
...
- Validate on the field's `change` event, which fires on blur, not on every `input` keystroke. A value the user is still typing almost always looks invalid partway through, so per-keystroke validation flashes an error on every character until the field is complete. Blur validation waits until the user signals they are done with the field. This matters most for format-masked fields like phone, where the value is incomplete until the final digit. Re-validate the whole form on submit to catch fields the user never focused.
- `canvas-input` does not validate itself. It only renders the error state you set. Validation logic, the rule and the message, lives in the consumer, which sets and clears the `error` attribute. This keeps validation rules in one place rather than split between the component and the page.
