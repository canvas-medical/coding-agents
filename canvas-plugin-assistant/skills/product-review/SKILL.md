...
7. **Audit trail / attribution** — the audit must reflect the *real actor* (session user or integration bot). Keep actor identity distinct from routing fields (`provider_id`, `assignee`): an integration naming a clinician for routing is fine as long as the audit stamps the bot as the actor. HIGH: API-key-triggered attestation events as a named clinician (electronic signing, signing-adjacent commits) and shadow-override patterns.
