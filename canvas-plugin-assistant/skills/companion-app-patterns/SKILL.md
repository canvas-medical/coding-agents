...
13. **Build WebSockets to survive daytime blue/green deploys.** Broadcasts are fire-and-forget and ephemeral — anything sent while the socket was down is lost, and the plugin process keeps no durable state across a restart. So treat the socket as a hint, not a source of truth: re-fetch the authoritative snapshot from the `SimpleAPI` endpoint on **every** (re)connect (wire it to the socket's `open` event, not just initial page load), reconnect with exponential backoff + jitter + cap (never a fixed delay — every client drops at once at cutover, so a fixed delay is a thundering herd against the freshly booted container), and reconnect immediately on the browser `online` event. Make mutating `POST`s idempotent (re-check state, no-op if already applied) since a deploy can drop the response and the client will retry. See `companion_app_patterns_context.txt` §7 "Surviving deploys & reconnection."

14. **Originate commands on the launching note (note-scope only).** Note-scope companion apps receive `note.id` in `self.event.context` and should pass it through the launcher URL to the handler. The handler uses it as `note_uuid` when constructing an SDK command (e.g. `VitalsCommand(note_uuid=…, …)`) and calling `command.originate()`. Return the resulting effect alongside the JSON response — the platform materializes the command in the note.
15. **Self-dismiss via the `INIT_CHANNEL` / `CLOSE_MODAL` contract.** The host transfers a `MessagePort` to the modal iframe via a `{type:'INIT_CHANNEL'}` postMessage with `event.ports[0]`. Store the port and, when done (e.g. after a successful submit), call `port.postMessage({type:'CLOSE_MODAL'})`. Fall back to `window.close()` if the port wasn't delivered. Don't invent alternate message shapes (`window.parent.postMessage`, lowercase `close`, etc.); stick to the documented contract.
...
All ship with detailed READMEs, 100% pytest coverage, and MIT licensing. The messaging plugin is the canonical reference for realtime push and deploy-resilient reconnection (rules 12–13); the vitals plugin is the canonical reference for note-scope command origination and the `INIT_CHANNEL` / `CLOSE_MODAL` dismiss contract (rules 14–15).
